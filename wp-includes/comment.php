 ...
 * @uses $wpdb
 *      
 * @param object|string|int $comment
 *        	Comment to retrieve.
 * @param string $output
 *        	Optional. OBJECT or ARRAY_A or ARRAY_N constants.
 ...
 *      
 * @param mixed $args
 *        	Optional. Array or string of options to override defaults.
...
;
		...
		if ('' !== $this->query_vars ['search']) {
			$where .= $this->get_search_sql ( $this->query_vars ['search'], array (
					'comment_author',
					'comment_author_email',
					'comment_author_url',
					'comment_author_IP',
 ...
 *        	Which timezone to use in reference to 'gmt', 'blog',
 *        	or 'server' locations.
...
function check_comment_flood_db($ip, $email, $date) {
	...
	if ($lasttime = $wpdb->get_var ( $wpdb->prepare ( "SELECT `comment_date_gmt` FROM `$wpdb->comments` WHERE `comment_date_gmt` >= %s AND ( `comment_author_IP` = %s OR `comment_author_email` = %s ) ORDER BY `comment_date_gmt` DESC LIMIT 1", $hour_ago, $ip, $email ) )) {
		...
		if ($flood_die) {
			...
			wp_die ( __ ( 'You are posting comments too quickly. Slow down.' ), '', array (
					'response' => 403 
			) );
		}
	}
...
function wp_delete_comment($comment_id, $force_delete = false) {
	...
	if (! $force_delete && EMPTY_TRASH_DAYS && ! in_array ( wp_get_comment_status ( $comment_id ), array (
			'trash',
			'spam' 
	) ))
 ...
 *        	Post content to check for links.
 * @param int $post_ID
 *        	Post ID.
 */
 ...
 *       
 * @param int|array $ids
 *        	Comment ID or array of comment IDs to remove from cache
 */
