 ...
 * @param mixed $default
 *        	Value to return if the query variable is not set. Default ''.
 * @return mixed
 */
 ...
 *      
 * @param mixed $post_types
 *        	Optional. Post Type or array of Post Types
 * @return bool
 */
...
class WP_Query {
	...
	public function get_posts() {
		...
		if (! empty ( $this->tax_query->queries )) {
			$tax_query_in_and = wp_list_filter ( $this->tax_query->queries, array (
					'operator' => 'NOT IN' 
			), 'NOT' );
			if (! empty ( $tax_query_in_and )) {
				if (! isset ( $q ['taxonomy'] )) {
					foreach ( $tax_query_in_and as $a_tax_query ) {
						if (! in_array ( $a_tax_query ['taxonomy'], array (
								'category',
								'post_tag' 
						) )) {
				...
				$cat_query = wp_list_filter ( $tax_query_in_and, array (
						'taxonomy' => 'category' 
				) );
				if (! empty ( $cat_query )) {
				...
				$tag_query = wp_list_filter ( $tax_query_in_and, array (
						'taxonomy' => 'post_tag' 
				) );
				if (! empty ( $tag_query )) {
