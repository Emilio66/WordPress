...
class WP_Posts_List_Table extends WP_List_Table {
	...
	protected function get_views() {
		...
		if (! empty ( $this->sticky_posts_count )) {
			...
			$sticky_link = array (
					'sticky' => "<a href='edit.php?post_type=$post_type&amp;show_sticky=1'$class>" . sprintf ( _nx ( 'Sticky <span class="count">(%s)</span>', 'Sticky <span class="count">(%s)</span>', $this->sticky_posts_count, 'posts' ), number_format_i18n ( $this->sticky_posts_count ) ) . '</a>' 
			);
...
<?php
		if ('top' == $which && ! is_singular ()) {
			...
			submit_button ( __ ( 'Filter' ), 'button', 'filter_action', false, array (
			...
			<?php
				...
				case 'title' :
					...
					if ($format = get_post_format ( $post->ID )) {
						...
						), 'edit.php' ) ) . '" class="post-state-format post-format-icon post-format-' . $format . '" title="' . $label . '">' . $label . ":</a> ";
					}
			...
			<div class="inline-edit-group">
								<label class="alignleft"> <span class="title"><?php _e( 'Password' ); ?></span>
									<span class="input-text-wrap"><input type="text"
										...
										value="" /></span>
								</label> <em style="margin: 5px 10px 0 0" class="alignleft">
					<?php
...
if (post_type_supports ( $screen->post_type, 'comments' ) || post_type_supports ( $screen->post_type, 'trackbacks' )) :
	...
	<?php
			...
			if ($bulk && current_theme_supports ( 'post-formats' ) && post_type_supports ( $screen->post_type, 'post-formats' )) {
				...
				?>
		<label class="alignleft" for="post_format"> <span class="title"><?php _ex( 'Format', 'post format' ); ?></span>
...
if (! $bulk) {
			...
			<?php
				...
				submit_button ( __ ( 'Update' ), 'button-primary alignright', 'bulk_edit', false, array (
						'accesskey' => 's' 
				) );
			}
			?>
